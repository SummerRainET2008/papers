...
\begin{document}
  ...
  \section{Methodology}
  In  this  section,  we  will  describe our proposed system of similarity model
  fused  with  classification  model,  named  as  SFC, in detail. We propose two
  approaches  to  implement  the  SFC idea, two-stageg SFC, and its more compact
  version called joint-SFC.
  ...
  process,  we have to first make sure that the ground truth class label $y_{i}$
  for  $x_{i}$  is  within  the  candidate  class  label  set $\mathcal{C'}$. If
  $y_{i}\notin  \mathcal{C'}$,  we will manually add $y_{i}$ into $\mathcal{C'}$
  ...
  \subsection{Multitask based training}
  The 2 tasks for stage 2 training process are listed below.

  \subsubsection*{Task 1: Regular sentence-pair similarity task}
  ...
  \subsubsection*{Task   2:  Classification  task  on  top-K  classes  based  on
  sentence-pair model}
  ...
  In  two-stage  SFC,  Stage 1 and stage 2 are separate, so they do not interact
  ...
  $\alpha_S$,   $\alpha_K$   and   $\alpha_C$   are  the  weights  for  task  1:
