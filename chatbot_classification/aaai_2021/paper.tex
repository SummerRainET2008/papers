...
\begin{document}
  ...
  In  stage  1,  we  can  use any auxiliary text classification model or tool,
  ...
  In  this  work,  we  choose pretrained models to fine tune on our data $\mathcal{D}$.
  ...
  In this stage, given a data point $x_{i}$ with
  ...
  We continue choosing RoBERTa as the main module in the sentence pair
  similarity  model.  

  
  Then, in stage 2, we will use sentence pair similarity
  model  to  identify  the class label with strongest semantic similarity to the
  user    input    sentence.    Suppose    we    have    a    training   dataset
  ...
  sentence  pair  grouped  by  2  user  input  queries and $l_{j}\in\{0, 1\}$
  denotes the similarity label, see Equ. \ref{eq:similarity label},
  \begin{align}
    l_{j}=
  ...
  the work \cite{bamler2020extreme}.  The  key  idea  is to
  ...
  auxiliary  model  or  tool in stage 1, where $K$ is a hyperparameter 
  to control  candidate  class  number,  we  apply  the adversarial negative
  ...
  The second task is \emph{classification  task  on  top-K  classes  based  on
  sentence-pair model}.
  ...
  interaction
  with  each other during training. In this case, the performance of stage 1 can
  limit  the  potential of stage 2, meanwhile, stage 2 also cannot give training
  feedback back to stage 1 for fine-tuning.
  ...
  \textbf{Recently-released intention detection datasets}.
