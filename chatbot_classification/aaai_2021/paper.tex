...
\begin{document}
  ...
  \begin{table*}
    \begin{centering}
      \scalebox{0.55}{
        \begin{tabular}{|c|cccccc|cccccc|cccccc|c|c|}
          ...
          (classification) & & & & & & & & & & & & & & & & & & & &\tabularnewline
          \hline
          RoBERTa-large & \multirow{2}*{$\textbf{0.8974i}$} &
          ...
          \multirow{2}*{$\textbf{0.9099}$} & \multirow{2}*{$i\textbf{0.9227}$} & 
    ...
    \end{centering}
    \caption{
      F-scores  on  five  task-specific  datasets text classification in chatbot
  ...
  All results in F-score (todo: reference) on the five datasets with different models and configurations are
  showed in Table  \ref{tbe:table2}.

  The performance of each model is measured in F-score on the fixed
  ...
  \subsubsection*{Is multi-task and joint training working?} 
