...
\begin{document}
  ...
  \begin{enumerate}
    ...
    recently  proposed label-embedding based LEAM (todo: reference). Empircally,
  ...
  Table  \ref{tbe:table2}  presents  all results on the five datasets 
  model  set-up  in detail. The performance of each model is measured in F-score
  on the fixed test set for each dataset. 
  Overall,  two-stage SFC systems show moderate improvements over all  baseline
  systems.  This  observation  gives  us  a  general  idea  that
  classification model and similarity model can actually provide complementary information
  to each other, and combining them together can enhance the overall capability 
  based on richer extracted semantic information.  
