  ...
  \begin{centering}
    \scalebox{0.58}{
      \begin{tabular}{|c|ccc|ccc|}
        ...
        \multirow{2}*{${0.9442}$} & \multirow{2}*{${0.9729}$} & 
...
As for the intent classification and NER joint model, we choose the state-of-the-art RoBERTa-large model with NER loss as our baseline. For ablation study, we did experiments on Joint SFC model with and without NER loss respectively. Our Joint SFC without NER loss still outperforms RoBERTa with NER loss baseline by 1.34 percent points on average. This indicates that the fusion of classification and similarity can even works better than utilizing additional NER information under few-shot chatbot scenario. Moreover, our Joint SFC with NER loss achieves further improvement by 2.80 percent points over RoBERTa with NER loss baseline. In this way, we show the compatibility of our Joint SFC model with extra semantic information.
