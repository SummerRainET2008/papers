...
We choose three kinds of system to construct our baselines. All our pretrained 
model baselines are being fine-tuned on all the experimental datasets. Additionally, 
the similarity model baseline will use the same fine-tuning process on Quora dataset  \cite{iyer2017first} as all our SFC models. As our researched problem is the application without sufficient data, the similarity model baseline is influenced due to insufficient in-domain data to a great extent. Thus, generally the similarity model baseline has to use external data.
